...
public enum DataProcess {
	...
	public void queryAndRank() {
		...
		File file3 = new File(ELROOT+"output/ranked_score/" + FILENAME);
		if(!file1.exists())
			file1.mkdirs();
		if(!file2.exists())
			file2.mkdirs();
	...
	private void searchE(String str, int i, int j) {
		...
		List<Integer> rank = generatePredict(i, j,originList);
	...
	//产生rank并排名
	private List<Integer> generatePredict(int i,int j,List<String> originList) {
		...
		if( file.exists() && file.isFile() ) {
			if(file.length() != 0) {
				...
				try {
					Process p = Runtime.getRuntime().exec(commends);
					p.waitFor();
					BufferedReader br = IO.Reader(ELROOT+"trainDataset/prediction/prediction"+i+j+".txt");
...
//					}
					...
					BufferedWriter score_br = IO.Writer(ELROOT+"output/ranked_score/"+FILENAME+"/e_"+i+"_"+j+".txt");
					//list中的index就是文档中candidate的index,其值是candidate的排名
					for (float f : map.keySet()) {
						list.add(map.get(f));
						//输出分数
						String rankedEntity = originList.get(map.get(f));
						score_br.write(rankedEntity+"\t\t"+f);
						score_br.newLine();
					}
					score_br.close();
